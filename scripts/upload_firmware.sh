...
function transfer_upload(){
	if [ "$1" == "cow" ]; then
		./transfer "cow" --block "2621440" -s -p "64" -t "180" "openwrt-${firmware_version}-rockchip-armv8-friendlyarm_nanopi-r2s-$2-sysupgrade.img.gz"
	elif [ "$1" == "wet" ]; then
		./transfer "wet" -s -p "16" -t "180" "openwrt-${firmware_version}-rockchip-armv8-friendlyarm_nanopi-r2s-$2-sysupgrade.img.gz"
	fi
}
...
if [ -z "${ext4_download_link}" ] && [ -z "${wet_download_link}" ]; then
	...
	openwrt_downlink="SquashFS Firmware:%0a${squashfs_download_link}"
elif [ -z "${squashfs_download_link}" ]; then
	openwrt_downlink="EXT4 Firmware:%0a${ext4_download_link}"
else
